...
let quizTimer;
let startTime;
let quizTimeLimit = 30; // Default
let hasAnswers = false; // Toggle state
let currentCardBgBase64 = null; // Store locally selected background
let currentQImgBase64 = null; // Store locally selected question image
let currentLang = 'en';

const translations = {
    en: {
        appTitle: "Quiz Creator",
        appSubtitle: "Design your interactive quiz below.",
        quizTitlePlaceholder: "My Awesome Quiz",
        importQuizBtn: "📂 Import Quiz",
        saveQuizBtn: "💾 Save Quiz",
        timeLimitLabel: "Time Limit per Question (sec): ",
        cardStyleLabel: "Global Card Style (Back): ",
        bgThemeLabel: "Select Background Theme (App)",
        defaultGradient: "Default Gradient",
        addQuestionHeader: "Add a New Question",
        questionTextLabel: "Question Text",
        questionPlaceholder: "Enter your question here...",
        questionImageLabel: "Image (Optional)",
        questionTypeLabel: "Question Type",
        typeNoAnswer: "Info / Open",
        typeWithAnswer: "Multiple Choice",
        ...
        choiceAOption: "Choice A",
        choiceBOption: "Choice B",
        choiceCOption: "Choice C",
        choiceDOption: "Choice D",
        addQuestionBtn: "Add Question",
        finishQuizBtn: "Finish & Play Quiz",
        questionProgress: "Question",
        nextQuestionBtn: "Next Question",
        quizCompletedHeader: "Quiz Completed!",
        quizCompletedDesc: "You have finished the quiz.",
        totalTimeLabel: "Total Time:",
        totalQuestionsLabel: "Total Questions:",
        createNewQuizBtn: "Create New Quiz",
        
        // Alerts & Dynamic
        alertEnterQuestion: "Please enter a question text.",
        alertFillAnswers: "Please fill in all 4 answer choices.",
        confirmDiscard: "The current question has incomplete answers. Discard it and finish?",
        alertAddOne: "Please add at least one question!",
        cardFrontQuestion: "Question"
    },
    ar: {
        appTitle: "منشئ الاختبارات",
        appSubtitle: "صمم اختبارك التفاعلي أدناه.",
        quizTitlePlaceholder: "عنوان الاختبار الخاص بي",
        importQuizBtn: "📂 استيراد اختبار",
        saveQuizBtn: "💾 حفظ الاختبار",
        timeLimitLabel: "الوقت لكل سؤال (ثانية): ",
        cardStyleLabel: "نمط البطاقة العام (الخلفية): ",
        bgThemeLabel: "اختر خلفية التطبيق",
        defaultGradient: "تدرج افتراضي",
        addQuestionHeader: "أضف سؤالاً جديداً",
        questionTextLabel: "نص السؤال",
        questionPlaceholder: "أدخل سؤالك هنا...",
        questionImageLabel: "صورة (اختياري)",
        questionTypeLabel: "نوع الوؤال",
        typeNoAnswer: "معلومة / مفتوح",
        typeWithAnswer: "متعدد الخيارات",
        ...
        choiceAOption: "الخيار أ",
        choiceBOption: "الخيار ب",
        choiceCOption: "الخيار ج",
        choiceDOption: "الخيار د",
        addQuestionBtn: "إضافة السؤال",
        finishQuizBtn: "إنهاء وبدء الاختبار",
        questionProgress: "السؤال",
        nextQuestionBtn: "السؤال التالي",
        quizCompletedHeader: "انتهى الاختبار!",
        quizCompletedDesc: "لقد أتممت الاختبار.",
        totalTimeLabel: "الوقت الكلي:",
        totalQuestionsLabel: "إجمالي الأسئلة:",
        createNewQuizBtn: "إنشاء اختبار جديد",

        // Alerts & Dynamic
        alertEnterQuestion: "الرجاء إدخال نص السؤال.",
        alertFillAnswers: "الرجاء ملء جميع خيارات الإجابة الأربعة.",
        confirmDiscard: "السؤال الحالي يحتوي على إجابات غير مكتملة. هل تريد تجاهله والإنهاء؟",
        alertAddOne: "الرجاء إضافة سؤال واحد على الأقل!",
        cardFrontQuestion: "سؤال"
    }
};

// 1. Fetch Background Images - REMOVED (User requested local file only for now, replacing dropdown)
// If we wanted both, we would keep fetchImages but that was for the dropdown.
// We will replace fetchImages logic with a listener for the file input.

// On Load
document.addEventListener('DOMContentLoaded', () => {
    // fetchImages(); // No longer needed as we removed the dropdown
    updateQuestionCountBadge();
    changeLanguage('en'); 

    // File Input Listener (App Background)
        ...
        bgInput.addEventListener('change', (e) => {
            ...
            if (file) {
                ...
                reader.onload = (evt) => {
                    ...
                    document.getElementById('background-layer').style.backgroundImage = `url('${bgData}')`;
                    document.getElementById('background-layer').style.backgroundSize = 'cover';
                    document.getElementById('background-layer').style.backgroundPosition = 'center';
                    
                    // We can save this to settings if we want it to persist in JSON
                    // We'll use a global variable to store it for export
                    window.currentAppBgBase64 = bgData;
                };
                reader.readAsDataURL(file);
            } else {
                 // Clear if cancelled or empty? Usually better to keep previous or reset
                 // document.getElementById('background-layer').style.backgroundImage = '';
            }
        });
    }

    // File Input Listener (Card Background)
    ...
    if(fileInput) {
        fileInput.addEventListener('change', (e) => {
            ...
            if (file) {
                const reader = new FileReader();
                reader.onload = (evt) => {
                    currentCardBgBase64 = evt.target.result;
                };
                reader.readAsDataURL(file);
            } else {
                currentCardBgBase64 = null;
            }
        });
    }

    // File Input Listener (Question Image)
    const qImgInput = document.getElementById('q-img-input');
    if(qImgInput) {
        qImgInput.addEventListener('change', (e) => {
            ...
            if (file) {
                const reader = new FileReader();
                reader.onload = (evt) => {
                    currentQImgBase64 = evt.target.result;
                };
                reader.readAsDataURL(file);
            } else {
                currentQImgBase64 = null;
            }
        });
    }
});
...
// 1. Fetch Background Images - REMOVED placeholder to avoid error if function is called separately. 
function fetchImages() {} 


// 2. Quiz Creation Logic
function toggleAnswerFields() {
    const isWithAnswer = document.querySelector('input[name="q-type"]:checked').value === 'with-answer';
...
function addQuestion() {
    ...
    if (!questionText) {
        alert(translations[currentLang].alertEnterQuestion);
        return;
    }
    ...
    if (type === 'with-answer') {
        ...
        if (!allFilled) {
            alert(translations[currentLang].alertFillAnswers);
            return;
        }

        correctAnswerIndex = parseInt(document.getElementById('correct-answer').value);
    }
    ...
    questions.push({
        ...
        qImg: currentQImgBase64
    });

    // Reset Form
    textInput.value = '';
    document.querySelectorAll('.answer-input').forEach(i => i.value = '');
    document.getElementById('correct-answer').value = "0"; // Reset to A
...
function changeLanguage(lang) {
    ...
    // Update Quiz Title Placeholder
    const qt = document.getElementById('quiz-title');
    if(qt) qt.placeholder = t.quizTitlePlaceholder;
    ...
    // Update specific elements if in quiz mode
    const qCountEl = document.getElementById('question-count');
    if (qCountEl && document.getElementById('quiz-view').style.display === 'flex') {
         qCountEl.textContent = `${t.questionProgress} ${currentQuestionIndex + 1} / ${questions.length}`;
    }
}

function updateQuestionCountBadge() {
    document.getElementById('question-count-badge').textContent = `#${questions.length + 1}`;
}

// --- Import / Export Logic ---

function saveQuiz() {
    const title = document.getElementById('quiz-title').value.trim() || translations[currentLang].quizTitlePlaceholder;
    const timeLimit = parseInt(document.getElementById('time-limit').value) || 30;
    
    // We strictly follow the User-Defined Structure
    const quizData = {
        title: title,
        meta: {
            createdAt: new Date().toISOString(),
            author: "User" // Could be dynamic if we had user profiles
        },
        settings: {
            language: currentLang,
            timeLimit: timeLimit,
            globalBackground: window.currentAppBgBase64 || null, // Capture App Background
            cardBackImage: currentCardBgBase64
        },
    ...
    reader.onload = (e) => {
        try {
            ...
            if (!data.questions || !Array.isArray(data.questions)) {
                alert("Invalid Quiz JSON: Missing 'questions' array.");
                return;
            }
            ...
            if (data.settings) {
                if (data.settings.language) {
                    document.getElementById('language-select').value = data.settings.language;
                ...
                if (data.settings.timeLimit) {
                    ...
                    quizTimeLimit = data.settings.timeLimit;
                }
                // Restore App Background
                if (data.settings.globalBackground) {
                     window.currentAppBgBase64 = data.settings.globalBackground;
                     document.getElementById('background-layer').style.backgroundImage = `url('${window.currentAppBgBase64}')`;
                     document.getElementById('background-layer').style.backgroundSize = 'cover';
                }
                // Restore Global Card Back
                if (data.settings.cardBackImage) {
                    currentCardBgBase64 = data.settings.cardBackImage;
                    // Note: We cannot easily restore the File Input UI (security), 
                    // but we have the Base64 in variable for rendering.
                }
            }
            ...
            updateQuestionCountBadge();
            alert(`Quiz imported successfully! Loaded ${questions.length} questions.`);
...
function finishAndPlay() {
    // Check for unsaved question draft
    const textInput = document.getElementById('question-text');
    const questionText = textInput.value.trim();
    ...
    if (questionText) {
        ...
        if (type === 'with-answer') {
            ...
            if (!isValid) {
                if (!confirm(translations[currentLang].confirmDiscard)) {
                    return;
                }
            } else {
                 correctAnswerIndex = parseInt(document.getElementById('correct-answer').value);
        ...
        if (isValid || type === 'no-answer') {
            questions.push({
             ...
             // Clear the input so it doesn't look weird if they come back
             textInput.value = '';
             document.querySelectorAll('.answer-input').forEach(i => i.value = '');
    ...
    if (questions.length === 0) {
        alert(translations[currentLang].alertAddOne);
        return;
    }
    ...
    quizTimeLimit = parseInt(timeInput.value) || 30;

    // Init Quiz Grid
    currentQuestionIndex = -1; // No specific question selected initially
    document.getElementById('total-q-num').textContent = questions.length;
    renderCardGrid();
}

// 3. Quiz Logic - Grid System
function startTimer() {
...
function handleQuestionComplete() {
    // 1. Find the focused card
    const focusedCard = document.querySelector('.flip-card.focused');
    if (!focusedCard) return;

    // 2. Animate out
    focusedCard.classList.remove('focused');
    focusedCard.classList.add('completed'); 

    // 3. Bring back other cards
    const others = document.querySelectorAll('.flip-card:not(.completed):not(.focused)');
    others.forEach(c => {
    ...
    setTimeout(() => {
        if(focusedCard.parentNode) {
            focusedCard.parentNode.removeChild(focusedCard);
        }
        
        // Check if all done
        const remaining = document.querySelectorAll('.flip-card');
...
function renderCardGrid() {
    ...
    questions.forEach((qData, index) => {
        ...
        flipCard.onclick = () => handleCardClick(flipCard, index);
        ...
        // Use Global Background if available
        if (currentCardBgBase64) {
            front.style.backgroundImage = `url('${currentCardBgBase64}')`;
        ...
        front.innerHTML = `
            <h2 style="${currentCardBgBase64 ? 'text-shadow: 0 2px 10px rgba(0,0,0,0.9); color: white;' : ''}">${index + 1}</h2>
            <span style="${currentCardBgBase64 ? 'text-shadow: 0 2px 10px rgba(0,0,0,0.9); color: white;' : ''}">${translations[currentLang].cardFrontQuestion}</span>
        `;
        ...
        /* Removed "No Choices" text block per requirement */

        back.appendChild(choicesGrid);
...
function handleCardClick(card, index) {
    ...
    // 1. Fade out others
    const allCards = document.querySelectorAll('.flip-card:not(.completed)');
    allCards.forEach(c => {
        if (c !== card) {
    ...
    // 2. Promote to Fixed Position for centering animation
    const rect = card.getBoundingClientRect();
    
    // Lock current position immediately to prevent jump
    ...
    // 3. Trigger Animation to Center
    requestAnimationFrame(() => {
        card.classList.add('focused');
        // Flip to reveal content
        card.classList.add('flipped');

        // Add Global Click Listener to close (Event Delegation on Document)
        setTimeout(() => {
            const closeHandler = (e) => {
                // If we click an answer, stopPropagation inside selectAnswer handles it. 
                // However, if we click background or non-interactive part...
                // e.target.closest('.choice-card') check is redundant if choice-card stops prop, 
                // but let's be safe.
                
                // If click is on a choice card, DO NOT CLOSE
        ...
        }, 100); // Small delay to avoid catching the opening click
    });
...
function selectAnswer(card, isCorrect) {
    if(card.classList.contains('selected') || card.classList.contains('wrong') || card.classList.contains('correct-highlight')) return;
    ...
    if (isCorrect) {
         ...
         // Could play sound or visual effect
    } else {
